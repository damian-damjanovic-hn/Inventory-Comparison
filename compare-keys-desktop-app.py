...
class InventoryGUI(tk.Tk):
    ...
    #     # Compare Columns (replacing Listbox with two Comboboxes)
    #     inter = sorted(set(cols_src).intersection(set(cols_tgt)))
    #     default_compare = [c for c in (self.compare_cols or []) if c in inter]
    #     default_1 = default_compare[0] if len(default_compare) > 0 else (inter[0] if inter else "")
    #     default_2 = default_compare[1] if len(default_compare) > 1 else (inter[1] if len(inter) > 1 else default_1)

    #     ttk.Label(container, text="Compare Column 1").grid(row=0, column=2, sticky="w")
    #     cmp1 = ttk.Combobox(container, values=inter, state="readonly")
    #     cmp1.set(default_1)
    #     cmp1.grid(row=1, column=2, sticky="ew", padx=(0, 8))

    #     ttk.Label(container, text="Compare Column 2").grid(row=2, column=2, sticky="w")
    #     cmp2 = ttk.Combobox(container, values=inter, state="readonly")
    #     cmp2.set(default_2)
    #     cmp2.grid(row=3, column=2, sticky="ew", padx=(0, 8), pady=(0, 8))
    ...
    #         self.tgt_key1 = tgt_k1.get().strip()
    #         self.tgt_key2 = tgt_k2.get().strip()
    #         self.compare_cols = [cmp1.get().strip(), cmp2.get().strip()]
    #         self.compare_cols = [c for c in self.compare_cols if c]  # remove empty selections
    #         if not self.compare_cols:
    #             messagebox.showwarning("Keys/Columns", "Please select at least one compare column.")
    #             return
    #         self.log_msg(f"Selected keys and columns. Compare: {self.compare_cols}")
    ...
    def select_keys_and_columns(self):
        ...
        inter = sorted(set(cols_src).intersection(set(cols_tgt)))
        if not inter:
            inter = cols_src[:2] if cols_src else cols_tgt[:2]  # fallback to first two from src or tgt

        default_compare = [c for c in (self.compare_cols or []) if c in inter]
        cmp1_d = default_compare[0] if len(default_compare) > 0 else (inter[0] if inter else "")
        cmp2_d = default_compare[1] if len(default_compare) > 1 else (inter[1] if len(inter) > 1 else cmp1_d)
