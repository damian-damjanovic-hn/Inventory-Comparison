...
class CompositeKeyDashboard:
    ...
    def _build_main_area(self, parent: ttk.Frame):
        ...
        self.nb.grid(row=0, column=0, sticky="nsew", pady=(0, 6))
        ...
        self.nb.add(self.tab_preview, text="Preview Table")
        self.tab_preview.rowconfigure(1, weight=1)
        self.tab_preview.columnconfigure(0, weight=1)

        topbar = ttk.Frame(self.tab_preview)
        topbar.grid(row=0, column=0, sticky="ew", pady=(0, 6))
        ttk.Label(topbar, text="Preview:").pack(side=LEFT)
        cb = ttk.Combobox(topbar, state="readonly", values=["ECommerce", "Warehouse"],
                        textvariable=self.current_preview_side, width=12)
        cb.pack(side=LEFT, padx=(6, 10))
        ttk.Button(topbar, text="Show 50 rows", command=self._preview_rows).pack(side=LEFT)

        self.preview_tree = ttk.Treeview(self.tab_preview, show="headings", bootstyle="info")
        self.preview_tree.grid(row=1, column=0, sticky="nsew")
        vsb = ttk.Scrollbar(self.tab_preview, orient="vertical", command=self.preview_tree.yview)
